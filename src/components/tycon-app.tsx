...
function speak(text: string, lang: LanguageCode, romanization?: string) { return <><span lang={lang}>{text}</span>{romanization && <small className="romanization" lang="zh-Latn-pinyin">{romanization}</small>}</>; }
...
export default function TyconApp({ initialView = "learn", requestedLessonId }: { initialView?: View; requestedLessonId?: string }) {
  ...
  useEffect(() => {
    ...
    if (saved.profile) {
      setSource(saved.profile.sourceLanguage); setTarget(saved.profile.targetLanguage); setGoal(saved.profile.dailyGoal);
  ...
  function completeOnboarding() {
    ...
    const now = new Date();
    const nextData: PersistedState = { ...emptyState(), profile: { id: crypto.randomUUID(), sourceLanguage: source, targetLanguage: target, uiLanguage: "en", dailyGoal: goal, createdAt: now.toISOString() } };
  ...
  function changePair() {
    ...
    const nextData: PersistedState = { ...data, activeSession: null, profile: { ...profile, sourceLanguage: source, targetLanguage: target, uiLanguage: "en", dailyGoal: goal } };
    setQuiz(freshQuiz()); setData(nextData); saveImmediately(nextData); navigate("learn"); setNotice("Your saved progress for other paths is safe.");
  }
  function quit() {
