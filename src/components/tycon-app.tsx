...
export default function TyconApp({ initialView = "learn", requestedLessonId }: { initialView?: View; requestedLessonId?: string }) {
  ...
  const [islandTheme, setIslandTheme] = useState<"classic" | "pixel">("classic");
  ...
  useEffect(() => {
    ...
    if (savedTheme === "pixel" || savedTheme === "classic") setIslandTheme(savedTheme);
  }, [hydrated]);
  useEffect(() => {
  ...
  function switchIslandTheme(next?: "classic" | "pixel") {
    const value = next ?? (islandTheme === "classic" ? "pixel" : "classic");
