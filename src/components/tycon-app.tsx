...
export default function TyconApp({ initialView = "learn", requestedLessonId }: { initialView?: View; requestedLessonId?: string }) {
  ...
  useEffect(() => {
    ...
    if (saved.profile) {
      ...
      if (initialView === "lesson") {
        ...
        setView(initialView);
        setQuiz(saved.activeSession ?? freshQuiz());
        if (initialView === "learn" && sessionStorage.getItem("tycon:learn-intro-seen") !== "1") setShowLearnIntro(true);
      }
    } else {
      setView("learn");
  ...
  function closeLearnIntro() { sessionStorage.setItem("tycon:learn-intro-seen", "1"); setShowLearnIntro(false); }
